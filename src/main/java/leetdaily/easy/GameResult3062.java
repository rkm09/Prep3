...
//    [def]; time: O(n), space: O(1) [faster]
    public static String gameResult(ListNode head) {
        ...
        return (oddCount != evenCount) ? (oddCount < evenCount ? "Even" : "Odd") : "Tie";
    }

    //    point Difference; time: O(n), space: O(1)
    public static String gameResult1(ListNode head) {
        ListNode currNode = head;
        int pointDifference = 0;
        while(currNode != null) {
            if(currNode.next != null) {
                pointDifference += currNode.val > currNode.next.val ? 1 : -1;
                currNode = currNode.next.next;
            }
        }
        return (pointDifference != 0) ? (pointDifference > 0 ? "Even" : "Odd") : "Tie";
    }
}
