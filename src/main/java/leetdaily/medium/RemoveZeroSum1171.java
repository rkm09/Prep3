...
public class RemoveZeroSum1171 {
    public static void main(String[] args) {
        ...
        ListNode result = removeZeroSumSublists1(head);
        while(result != null) {
...
//    prefix sum & hashmap (2 pass); time: O(n), space: O(n)
...
//      2nd pass:  delete consecutive sequences of zero sum
    ...
    //    prefix sum & hashmap (1 pass); time: O(n), space: O(n)
    public static ListNode removeZeroSumSublists1(ListNode head) {
        ...
        while(current != null) {
            prefixSum += current.val;
//            if prefix sum is already in the hashmap zero sum sequence has been found;
            if(prefixSumToNode.containsKey(prefixSum)) {
                ListNode prev = prefixSumToNode.get(prefixSum);
                current = prev.next;
                int p = current.val + prefixSum;
//                delete zero sum nodes from the hashmap
                while(p != prefixSum) {
                    prefixSumToNode.remove(p);
                    current = current.next;
                    p += current.val;
                }
//                make the connection from node before zero sum to the node after
