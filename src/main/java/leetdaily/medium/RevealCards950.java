...
//    two pointer; time: O(nlogn), space: O(logn) [for sort storage]
...
//        maintain a sorted order of deck cards
        Arrays.sort(deck);

        while(indexInDeck < n) {
//            if there is a gap available
            if(result[indexInResult] == 0) {
//                if not a skip position
                if(!skip) {
                    result[indexInResult] = deck[indexInDeck++];
                }
//                toggle between skip and not skip
                skip = !skip;
            }
//            may have to go back and forth
